...
// Public contracts require a bounded request id on every request. It is
// separate from the idempotency key so retries remain independently traceable.
export function requestId() {
  requestSeq += 1;
  return `k6-${NONCE}-${__VU}-${__ITER}-${requestSeq}`.slice(0, 64);
}
...
function opts(tag, extra) {
  ...
  o.headers = Object.assign({ 'X-Request-Id': requestId() }, o.headers || {});
  return o;
}
