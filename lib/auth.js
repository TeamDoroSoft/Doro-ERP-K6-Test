...
export function ensureKiosk(opts) {
  ...
  const secrets = KIOSK_SECRETS.split(',').map((v) => v.trim()).filter(Boolean);
  const deviceIndex = ((__VU - 1 + (o.offset || 0)) % KIOSK_COUNT);
  const configured = secrets[deviceIndex] || KIOSK_SECRET;
  // Accept either the raw secret expected by activate or the one-time full
  // `kdc_<id>.<secret>` value printed by device registration.
  const secret = configured.startsWith('kdc_') && configured.includes('.')
